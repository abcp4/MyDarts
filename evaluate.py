...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        # test
        print("###################TEST#########################")
        _,_,preds,targets= validate(test_loader, model, arch,epoch, cur_step,overall = True,debug=True)
        s = [preds,targets]
        import pickle
...
def train(train_loader, valid_loader, model, arch, w_optim, alpha_optim, lr, epoch):
    ...
    for step, ((trn_X, trn_y), (val_X, val_y,z)) in enumerate(zip(train_loader, valid_loader)):
